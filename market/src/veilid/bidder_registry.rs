...
impl BidderRegistryOps {
    ...
    pub async fn register_bidder(
        ...
        for attempt in 0..max_retries {
            ...
            let old_value = routing_context.get_dht_value(listing_key.clone(), Self::BIDDER_REGISTRY_SUBKEY, true).await?;
            ...
            // Try to write
            match routing_context.set_dht_value(listing_key.clone(), Self::BIDDER_REGISTRY_SUBKEY, data, None).await {
                Ok(returned_value) => {
                    // Check for concurrent modifications
                    if let Some(rv) = returned_value {
                        if Some(rv.seq()) != old_seq {
                            warn!("Bidder registry conflict (attempt {}/{}), retrying...", attempt + 1, max_retries);
                            tokio::time::sleep(retry_delay).await;
                            retry_delay *= 2;
                            continue;
                        }
                    }
                    info!("Successfully registered as bidder (attempt {}/{})", attempt + 1, max_retries);
                    let _ = routing_context.close_dht_record(listing_key.clone()).await;
                    return Ok(());
                }
                Err(e) => {
                    warn!("Failed to write bidder registry (attempt {}/{}): {}", attempt + 1, max_retries, e);
                    ...
                    let _ = routing_context.close_dht_record(listing_key.clone()).await;
                    return Err(anyhow::anyhow!("Failed to register bidder after {} attempts: {}", max_retries, e));
                }
            }
        }

        let _ = routing_context.close_dht_record(listing_key.clone()).await;
        Err(anyhow::anyhow!("Failed to register bidder after {} retries", max_retries))
    }
    ...
    pub async fn fetch_bidders(&self, listing_key: &RecordKey) -> Result<BidderRegistry> {
        ...
        match routing_context.open_dht_record(listing_key.clone(), None).await {
            Ok(_) => {
                match routing_context.get_dht_value(listing_key.clone(), Self::BIDDER_REGISTRY_SUBKEY, true).await? {
                    Some(value) => {
                        let registry = BidderRegistry::from_cbor(value.data())?;
                        info!("Fetched bidder registry with {} bidders", registry.bidders.len());
                        let _ = routing_context.close_dht_record(listing_key.clone()).await;
                        Ok(registry)
                    }
                    None => {
            ...
            Err(e) => {
                Err(anyhow::anyhow!("Failed to open listing record: {}", e))
            }
        }
    }
}
